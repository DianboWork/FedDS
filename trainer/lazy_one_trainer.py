...
class Lazy_One_Trainer(nn.Module):
    def __init__(self,
                 model,
                 train_path,
                 val_path,
    ...
    def train_model(self):
        ...
        for epoch in range(self.max_epoch):
            ...
            user_data = [[]] * self.args.num_users
            max_instance = dict()
            for idx in tqdm(user_idxs):
                max_instance = self.model.get_max_instance(self.train_loader[idx], idx, max_instance, self.args)
            for ins_name in max_instance:
                idx = max_instance[ins_name][2]
                user_data[idx].append(self.train_loader[idx].get_instance(ins_name, max_instance[ins_name][1]))
            for idx in user_idxs:
                print("Training on local data of user %s." % (idx), flush=True)
                local_model = copy.deepcopy(self.model) # GPU
                local = LocalUpdate(self.args, local_model, lr)
                local_out = local.train_local_model(user_data[idx])
    ...
    def eval_model(self, eval_loader, model):
        ...
        with torch.no_grad():
            ...
            print('Mean: %.4f' % ((float(prec[99]) + float(prec[199]) + float(prec[299])) / 3), flush=True)
...
class LocalUpdate(object):
    def __init__(self, args, model, lr):
        self.args = args
        # Model
        self.model = model
        if self.args.use_gpu:
    ...
    def batchify(self, batch_instance):
        label = torch.Tensor([ele[0] for ele in batch_instance]).long()
        tokens = torch.Tensor([ele[1] for ele in batch_instance]).long()
        pos1 = torch.Tensor([ele[2] for ele in batch_instance]).long()
        pos2 = torch.Tensor([ele[3] for ele in batch_instance]).long()
        mask = torch.Tensor([ele[4] for ele in batch_instance]).long()
        if self.args.use_gpu:
            tokens = tokens.cuda()
            pos1 = pos1.cuda()
            pos2 = pos2.cuda()
            label = label.cuda()
            mask = mask.cuda()
        return label, tokens, pos1, pos2, mask

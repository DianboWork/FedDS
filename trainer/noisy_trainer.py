        ...
        for epoch in range(self.max_epoch):
            ...
            if current_auc > best_auc:
                ...
                with open(self.ckpt+"/result/%s_frac_%.2f_epoch_%d_auc_%.4f.json" %(self.model.name, self.args.frac, epoch, result['auc']), 'w') as f:
                    out_dict = {"prec": result["prec"], "rec": result["rec"]}
    ...
    def train_local_model(self):
        for epoch in range(self.args.local_epoch):
            ...
            for batch_id in range(total_batch):
                ...
                if not batch_bags:
                    continue
                token, pos1, pos2, mask, scope, label, _ = self.batchify(batch_bags, self.args.use_gpu)

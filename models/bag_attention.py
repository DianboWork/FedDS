...
class BagAttention(nn.Module):
    ...
    def __init__(self, sentence_encoder, num_class, rel2id):
        ...
        """
        super().__init__()
        print("Building Bag Attention ...", flush=True)
        self.name = "Bag_Attention"
        ...
        for item in bag:
            if 'text' in item:
                token, pos1, pos2, mask = self.tokenizer(item['text'],
                                                         item['h']['pos'], item['t']['pos'], is_token=False,
                                                         padding=True)
            else:
                token, pos1, pos2, mask = self.tokenizer(item['token'],
                                                         item['h']['pos'], item['t']['pos'], is_token=True,
                                                         padding=True)
            tokens.append(token)
            pos1s.append(pos1)
            pos2s.append(pos2)
            ...
            if bag_size == 0:
                ...
                for i in range(len(scope)):
                    ...
                    softmax_att_score = self.softmax(
                        att_score[scope[i][0]:scope[i][1]].transpose(0, 1))  # (N_class, (softmax)n)
                    rep_for_each_rel = torch.matmul(softmax_att_score, bag_mat)  # (N_class, n) * (n, H) -> (N, H)
                    logit_for_each_rel = self.softmax(self.fc(rep_for_each_rel))  # ((each rel)N_class, (logit)N)
                ...
                softmax_att_score = self.softmax(att_score.transpose(1, 2))  # (B, N_class, (softmax)bag)
                rep_for_each_rel = torch.matmul(softmax_att_score, rep)  # (B, N_class, bag) * (B, bag, H) -> (B, N_class, H)
                bag_logits = self.softmax(self.fc(rep_for_each_rel)).diagonal(dim1=1, dim2=2)  # (B, (each rel)N_class)
